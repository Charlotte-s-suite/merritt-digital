...
| `oak-luminous` @ `609dd91` | the luminous-line rebuild, cut before `apply.html` existed | superseded by the PR branch below |
| **`oak-luminous-pr`** | **the same work rebuilt on current `main`** so it carries `apply.html` forward instead of deleting it — see §8 | open PR |
| `apply-intake` → PR #2 | `apply.html`, the public no-login application, shipped disabled | **merged** 2026-08-02 |
| `apply-live` → PR #3 | the enable-flip once the Access bypass landed | **merged** 2026-08-05 |

⚠ **`oak-luminous` was cut from a commit that predates PR #1's squash into `main`.** A PR opened
straight off it shows already-merged files as new AND deletes `apply.html`, because that page did
not exist on the branch. That is why the PR goes out from `oak-luminous-pr`, built on current
`main`. Anything else long-lived off an old fork point needs the same treatment — check
`git diff --name-status origin/main <branch>` for stray `D` lines before opening.
